...
module.exports = function(model,config){	
    ...
    module.forgot = async function(request, response){
        ...
        let ip = request.connection.remoteAddress.replace(/^.*:/, '');
            try {
                    let sql = "SELECT userId,mobile,countryCode,pin FROM " + config.Table.USER + " WHERE userName=" + sequelize_luckynumberint.escape(inputs.username) + " AND platform=" + sequelize_luckynumberint.escape('lottobets') + " ORDER BY created_at DESC limit 1";
                    let result = await sequelize_luckynumberint.query(sql, { transaction: tra_lucky ,type: sequelize_luckynumberint.QueryTypes.SELECT});
