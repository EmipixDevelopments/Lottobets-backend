...
module.exports = function(model,config){    
    ...
    module.watchNextDraw = async function(request, response){
            ...
            try {
                ...
                let sql = "SELECT le.profileID,le.ID,DATE_FORMAT(DATE_ADD(le.DrawTime,INTERVAL (-1 *TimeZone)+2 HOUR),'%Y-%m-%d %H:%i:%s') as DrawTime,ll.TimeZone, le.Result FROM " + config.Table.LOTTOLIST + " ll LEFT JOIN " + config.Table.LOTTOEVENT + " le ON  ll.ID=le.ProfileID WHERE  le.IsClosed=1 AND le.Result!='' AND  le.DrawTime >= '"+next+"' AND le.DrawTime <= '"+current+"' ORDER BY le.ID DESC";
                console.log(sql)
            ...
            try {
                ...
                if(profileIDArr.length){
                    ...
                    console.log(next_sql)
                        ...
                        for(let i=0;i<result.length;i++){
                            for(j=0;j<filter_result.length;j++){
                                console.log('pre=',result[i].lottoId,'==',filter_result[j].ProfileID);
