    ...
    module.watchNextDraw = async function(request, response){
            ...
            var current = new Date(dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss",true));
               ...
               // t.setDate(t.getDate() + 2);
                ...
                next.setHours(next.getMinutes() + 5);
                next = dateFormat(next, "yyyy-mm-dd HH:MM:ss");
                console.log("current===",current);
                console.log("next===",next);
            let tra = await sequelize_cngapi.transaction();
            try {
                let sql = "SELECT le.ProfileID AS lottoId,le.ID AS lottoEventId,le.Description,ll.ProfileName,ll.State,ll.Country,ll.drawLink,ll.RegUsed,ll.StartNum,ll.live_url,cl.Id AS CountryId,cl.FlagAbv As countryFlag,ll.colorimage,ll.grayscaleimage,DATE_FORMAT(DATE_ADD(le.DrawTime,INTERVAL (-1 *TimeZone)+2 HOUR),'%Y-%m-%d %H:%i:%s') as DrawTime,ll.TimeZone,cl.Continent, DATE_FORMAT(DATE_ADD(le.CutTime,INTERVAL (-1 *TimeZone)+2 HOUR),'%Y-%m-%d %H:%i:%s') as CutTime,le.Result FROM " + config.Table.LOTTOLIST + " ll LEFT JOIN " + config.Table.LOTTOEVENT + " le ON  ll.ID=le.ProfileID LEFT JOIN " + config.Table.CUNTRYLIST + " cl ON ll.CountryId=cl.Id WHERE DATE_ADD(le.CutTime,INTERVAL (-1 *TimeZone)+2 HOUR)>='" + current + "' AND DATE_ADD(le.CutTime,INTERVAL (-1 *TimeZone)+2 HOUR)<='" + next + "' AND ll.Enable=1 and le.DrawTime <= now() - interval 8 day AND le.IsClosed!=1 GROUP BY le.ProfileID ORDER by DATE_ADD(le.CutTime,INTERVAL (-1 *TimeZone)+2 HOUR)";
                ...
                for(let i=0;i<result.length;i++){
                    ...
                    let date1 = new Date(dateFormat(result[i].DrawTime, "yyyy-mm-dd"));
                        let date2 = new Date(dateFormat(new Date(), "yyyy-mm-dd"));
                        let diffTime = Math.abs(date2 - date1);
                        let diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
                        console.log(diffTime + " milliseconds");
                        console.log(diffDays + " days");
                        if(diffDays<=8 && result[i].Result!=''){
                            result[i]['lastDrawTime'] = result[i].DrawTime;
                            result[i]['lastResult'] = result[i].Result;
                            dataArr.push(result[i]);
                        }else{
                            //result[i]['lastDrawTime'] = '';
                        }

                     /*sql = "SELECT le.ProfileID,DATE_FORMAT(DATE_ADD(le.DrawTime,INTERVAL (-1 *TimeZone)+2 HOUR),'%Y-%m-%d %H:%i:%s') as DrawTime,ll.TimeZone, le.Result FROM " + config.Table.LOTTOEVENT + " le WHERE le.ProfileID='" + result[i].lottoId + "' le.DrawTime <= now() - interval 8 day ORDER BY le.DrawTime DESC limit 1";
